        ...
        @foreach ($tabs as $key => $group)
            <button wire:key="{{ $key }}" wire:click="selectedTab('{{ $group }}')"
                ...
                <div class="flex items-center h-full pl-4">
                    <span class="text-base font-semibold">{{ __($group) }}</span>
                </div>
            </button>
        @endforeach
    </div>
    @foreach ($settingGroups as $group => $items)
        @if ($activeTab === $group)
            <div wire:key="{{ $group }}"
                ...
                <h2 class="text-xl font-bold">{{ __($group) }}</h2>
                <div id="sortable">
                        ...
                        <div wire:key="{{ $group . $key }}" data-id="{{ $item['id'] }}"
                            ...
                            <label
                                ...
                                <button wire:loading.attr="disabled" wire:loading.class="!bg-primary-200"
                                    ...
                                    <x-heroicon-o-trash
                                        onclick="return confirm('{{ __('confirm') }}') ? Livewire.emit('destroySetting',{{ $item['id'] }}) : false"
                                        class="w-5 h-5 hover:text-danger-500 cursor-pointer" />
                                </button>
                                @endif
                            </label>
                            @includeIf('filament.pages.formfields.' . $item['type'], [
    ...
    <div
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-2">
            ...
            <select wire:model.lazy="newSetting.type"
                ...
                <option value="">
                    {{ __('default') }}</option>
                <option value="text">{{ __('text') }}
                </option>
                <option value="text_area">
                    {{ __('text_area') }}</option>
                <option value="image">{{ __('image') }}
                </option>
            </select>
